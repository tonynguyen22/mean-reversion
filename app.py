...
if uploaded_file:
    try:
        ...
        summary = {
            ...
            'Avg T+90 Return (%)': round(np.mean(returns['T+90']), 2) if returns['T+90'] else 'N/A',
            'Avg T+180 Return (%)': round(np.mean(returns['T+180']), 2) if returns['T+180'] else 'N/A',
            'Avg T+360 Return (%)': round(np.mean(returns['T+360']), 2) if returns['T+360'] else 'N/A',
        }
